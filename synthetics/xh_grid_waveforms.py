...
# rd		reflection depth (km) of the reverberation phase
    ...
    stack_seism = np.zeros((180,int((cut_a+cut_b)/dt)))
    average = np.zeros(int((cut_a+cut_b)/dt))
    stack_num = np.zeros(180)
    # mark reverberation phases
    
    os.chdir('{}'.format(dirname))
    fig3,ax3 = plt.subplots(1,1,figsize=[7,11])
...
#                if plot == 'True' :
                    ...
                    N2 = N1 + int((cut_b+cut_a)/delta)
                    t = np.linspace(-1*cut_b,cut_a,num=len(amp[N1:N2]))
                    if normalize == 'True':
                        norm = np.array(amp[N1:N2])/amp[int((-b+tpck[3])/delta)]
                        average = average + norm
                        ax3.plot(np.linspace(-cut_b,cut_a,num=len(average)),norm+flt[yy_index],lw=0.5,c='k',alpha=0.7)
#                        norm = norm/np.sqrt(np.abs(norm))  # root stack
                        stack_seism[yy_value,:] = stack_seism[yy_value,:] + norm
                        stack_num[yy_value] = stack_num[yy_value] + 1
            else:
                break
    ax3.set_xlabel('Times (s)',size=12)
    ax3.set_ylabel('Distance (deg)',size=12)
    fig3.tight_layout()
    fig3.savefig('{}/{}.traces.pdf'.format(odirname,filename))
    fp.close()

    ylim_min = np.where(stack_num>0)[0][0]
    ylim_max = np.where(stack_num>0)[0][-1]
    average = average/np.sum(stack_num)
    fig2,ax2 = plt.subplots(1,1,figsize=[8,3])
    ax2.plot(np.linspace(-cut_b,cut_a,num=len(average)),average,lw=1,c='k')
    ax2.plot([0,0],[-0.3,0.3],'--',c='k',alpha=0.5)
    ax2.set_xlabel('Time (s)',size=10)
    ax2.set_ylabel('Amp',size=10)
    ax2.set_xlim([-50,300])
    ax2.set_ylim([-0.05,0.05])
    fig2.tight_layout()
    fig2.savefig('{}/{}.average.pdf'.format(odirname,filename))
    for i in np.arange(180):
        if stack_num[i] > 0:
            stack_seism[i,:] = stack_seism[i,:] / np.float(stack_num[i])
#            stack_seism[i,:] = root_square(stack_seism[i,:])
#            ax.plot(np.arange(-cut_b,cut_a,delta),stack_seism[i,:]+i,lw=0.1,c='k')
            ax.fill_between(np.linspace(-cut_b,cut_a,num=len(average)),stack_seism[i,:]*10+i, i,where=(stack_seism[i,:] > 0), facecolor='r')
            ax.fill_between(np.linspace(-cut_b,cut_a,num=len(average)),stack_seism[i,:]*10+i, i,where=(stack_seism[i,:] < 0), facecolor='b')
...
#    cm = ax.imshow(stack_seism,cmap='bwr',vmin=-0.1,vmax=0.1,aspect='auto')
    ...
    ax.plot([0,0],[50,120],'k-',lw=0.5,alpha=0.5)
...
def main():
    ...
    path = '/home/meichen/work1/SH_TOMO/synthetics/XH_files/pointcheck'
    current_path = '/home/meichen/Research/SH_TOMO/synthetics'
    
    dirname = '{}'.format(path)
    fig,ax = plt.subplots(1,1,figsize=[7,11])
    filename = "010300X.-127.37.625.t3"
    readxh(ax=ax,filename=filename,dirname=dirname,odirname=current_path,plot='True',align_index=9,phase='S',yy_index=6,ylabel='Distance (deg)',cut_b=300,cut_a=50,ofile='{}.pdf'.format(filename),normalize='True',color_seism='black',dt=0.063)
