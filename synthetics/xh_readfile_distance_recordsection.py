...
# rd		reflection depth (km) of the reverberation phase
    ...
    stack_seism = np.zeros((1800,int((cut_b+cut_a)/dt)))
...
#            ax.fill_between(np.arange(3500)*0.1-50, stack_seism[i,:]+i*0.1+70, i*0.1+70,where=(stack_seism[i,:] < 0), facecolor='b')
    ...
    # plot record section
    cm = ax.imshow(stack_seism,cmap='bwr',vmin=-0.5,vmax=0.5,aspect='auto')
    ax.tick_params('x',bottom=True,top=True)
#    ax.set_xticks([0,int(cut_b/delta),int((cut_b+50)/delta),int((cut_b+100)/delta),int((cut_b+150)/delta),int((cut_b+200)/delta),int((cut_b+250)/delta),int((cut_b+300)/delta)])
    ax.set_xticks([0,int(cut_b/delta),int((cut_b+50)/delta),int((cut_b+60)/delta),int((cut_b+70)/delta),int((cut_b+80)/delta),int((cut_b+90)/delta),int((cut_b+95)/delta),int((cut_b+97)/delta),int((cut_b+100)/delta),int((cut_b+140)/delta),int((cut_b+145)/delta),int((cut_b+147)/delta),int((cut_b+150)/delta),int((cut_b+200)/delta),int((cut_b+250)/delta),int((cut_b+300)/delta)])
    ax.set_xticklabels(['{:d}'.format(-cut_b),'0','50','60','70','80','90','95','97','100','140','145','147','150','200','250','300'])
...
#    time_resp_S = rt[:,4]
#
#    event_depth_410 = event_depth[np.abs(reflection_depth-400)<0.1]
#    time_resp_S_410 = time_resp_S[np.abs(reflection_depth-400)<0.1]
#    distance_410 = distance[np.abs(reflection_depth-400)<0.1]
#    time_resp_S_410 = time_resp_S_410[np.abs(event_depth_410-10)<0.1]
#    distance_410 = distance_410[np.abs(event_depth_410-10)<0.1]
#    distance_410 = distance_410[time_resp_S_410>0.0]
#    time_resp_S_410 = time_resp_S_410[time_resp_S_410>0.0]
#    ax.plot(time_resp_S_410/dt+50/dt,distance_410*10,color='k',lw=0.1,alpha=0.1)
#    event_depth_660 = event_depth[np.abs(reflection_depth-660)<0.1]
#    time_resp_S_660 = time_resp_S[np.abs(reflection_depth-660)<0.1]
#    distance_660 = distance[np.abs(reflection_depth-660)<0.1]
#    time_resp_S_660 = time_resp_S_660[np.abs(event_depth_660-10)<0.1]
#    distance_660 = distance_660[np.abs(event_depth_660-10)<0.1]
#    distance_660 = distance_660[time_resp_S_660>0.0]
#    time_resp_S_660 = time_resp_S_660[time_resp_S_660>0.0]
#    ax.plot(time_resp_S_660/dt+50/dt,distance_660*10,color='k',lw=0.1,alpha=0.1)
...
def main():
    ...
    path = '/home/meichen/work1/SH_TOMO/synthetics/XH_files/S40RTS_NOCRUST_10s'
    current_path = '/home/meichen/Research/SH_TOMO/synthetics'
    
    dirname = '{}'.format(path)
    fig,ax = plt.subplots(1,1,figsize=[7,9])
    filename = "010200X.dis"
    rtfilename = "rt.prem_ml..s"
    readxh(ax=ax,filename=filename,rtfilename=rtfilename,dirname=dirname,odirname=current_path,plot='True',align_index=3,phase='S',yy_index=6,ylabel='Distance (deg)',cut_b=50,cut_a=350,ofile='{}.pdf'.format(filename),normalize='True',color_seism='black',dt=0.063)
