...
#    print(len(grid_j),target_depth)
    ...
    delta0_list = np.array([0.5,1,2,3])
    fig,ax = plt.subplots(1+len(delta0_list),1,figsize=[4,2.5*len(delta0_list)+2.5])
    misfitreduce = []
    # vary delta0
    for index,delta0 in enumerate(delta0_list):
        ...
        ax[index].set_title(r'$delta_0$={}'.format(delta0))
    ...
    # plot misfitreduce as a function of delta0
    ax[-1].scatter(delta0_list,misfitreduce,s=3,color='k')
    ax[-1].set_xlabel(r'$Delta_0$')
    ax[-1].set_ylabel('Reduced misfit (%)')
    fig.tight_layout()
    plt.savefig('{}/solve_matrix_onedep_Delta.pdf'.format(savepath))
