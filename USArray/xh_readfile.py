        ...
        while True:
            ...
            if xhhead:
                ...
                if plot == 'True':
                    ...
                    tstart_utc = UTCDateTime("{}-{}-{}T{}:{}:{}".format(tstart[0],tstart[1],tstart[2],tstart[3],tstart[4],int(tstart[5])))
                    b = tstart_utc - ot_utc
                    e = b + delta*ndata
                    ...
                    if normalize == 'True':
                        norm = np.array(amp[N1:N2])/amp[int((tpck[3]-b)/delta)]
#                        norm = norm/np.sqrt(np.abs(norm))
                        ax.plot(t,norm + yy_value,lw=0.05,c='{}'.format(color_seism),alpha=0.5)
#                        ax.fill_between(t,norm+yy_value,yy_value,where=(norm>0),facecolor='r')
#                        ax.fill_between(t,norm+yy_value,yy_value,where=(norm<0),facecolor='b')
                    elif normalize == 'False':
                        ax.plot(t,np.array(amp[N1:N2])+yy_value ,lw=0.4,c='{}'.format(color_seism),alpha=0.3)
                        #ax.scatter((tpck[10]-align_time)/delta,amp[int((-b+tpck[10])/delta)],marker='^',color='green',s=100)
                    else:
...
def main():
    ...
    path = '/home/meichen/work1/SH_TOMO'
...
#    events_selected = np.array(events_selected)
    for eventid in ['111416A']:
        print(eventid)
        dirname = '{}/events/event_{}/waveforms/XH_files'.format(path,eventid)
        fig,ax = plt.subplots(1,1,figsize=[5,12])
        filename = "{}.BHT.vel.snrS.xh".format(eventid)
        readxh(ax=ax,filename=filename,dirname=dirname,odirname=current_path,plot='True',align_index=3,phase='S',yy='intg',yy_index=6,ylabel='Distance (deg)',cut_b=50,cut_a=350,ofile='{}.pdf'.format(filename),normalize='False',color_seism='black')
...
def single_gridpoint():
    ...
    dirname = '/home/meichen/work1/SH_TOMO/Combined_xh/pointcheck'
    fig,ax = plt.subplots(1,1,figsize=[9,12])
    filename = "grid.lat_40.lon-120.dep_450"
    readxh(ax=ax,filename=filename,dirname=dirname,odirname=current_path,plot='True',align_index=9,phase='SSvDEPs',yy='flt',yy_index=6,ylabel='Distance (deg)',cut_b=350,cut_a=50,ofile='{}.pdf'.format(filename),normalize='False',color_seism='black')
