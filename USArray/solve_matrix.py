...
# stack_num		the minimum number of stacked seismograms of each grid point
    ...
    #dep = np.hstack((np.arange(355,505,5),np.arange(605,755,5)))
    dep = np.arange(400,500,5)
    ...
    for target_depth in dep:
        ...
        with open("{}".format(filename),"rb") as fp:
            while True:
                ...
                if bihead:
                    ...
                    if (np.abs(np.array(head[2])-target_depth)<0.1) and head[6] < 2.0 and head[8]<110 and head[8]>50:
        ...
        A_evt = coo_matrix((value_evt,(row_evt,col_evt)),shape=(m_evt,number_grid+number_event))
...
def main():
    filename = 'rp_Asia_interval1_area_deg3_win10_400_495_dep5.BHT.vel.snrS.xh'
    xhfilename = 'all.BHT.vel.snrS.xh'
