...
#    lamda_evt = 0.0/len(np.where(count>50)[0])
#    # Regularization matrix of event terms
#    lamda_grid = 0.0/number_grid
#    row_evt = []
#    col_evt = []
#    value_evt = []
#    m = 0
...
#    rhs = np.concatenate((rhs0,rhs_reg,rhs_evt),axis=0)
    ...
    extract_index_0 = np.hstack((np.where(count>5)[0],np.arange(number_grid,number_grid+number_event)))
...
def main():
    ...
    xhfilename = 'all.BHT.dis.snrS2.xh.gcarc60110.20evt'
    dirname = '/home/meichen/work1/SH_TOMO/Combined_xh'
    curpath = '/home/meichen/Research/SH_TOMO/USArray/vel_run'
    minseismnum = 20
    ...
    for target_depth in np.arange(10,1001,5):
        ...
        readbinary(filename=filename,dirname='{}/results_all_deg2_win5_dis_inverse_ttcorr'.format(dirname),savepath=curpath,evt=evt,seismnum=seismnum,minseismnum=minseismnum,target_depth=target_depth,average_or_root=average_or_root)
