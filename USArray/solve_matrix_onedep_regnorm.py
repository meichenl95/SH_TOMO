    ...
    with open("{}".format(filename),"rb") as fp:
        while True:
            ...
            if xhhead:
                ...
                cmtcd = head[216]
                if flt[0]>10 and flt[1]>5 and flt[2]>5 and flt[3]>2.5:
    ...
    with open("{}".format(filename),"rb") as fp:
        while True:
            ...
            if bihead:
                ...
                if ( np.abs(np.array(head[2])-target_depth) <0.1 and head[6]<2 and head[9]>10 and head[10]>5 and head[11]>5 and head[12]>2.5):
                    ...
                    elif (average_or_root == 'root'):
                        results.append(np.float(head[13])/np.sqrt(abs(head[13])))
                    gcarc.append(head[6])
                    az.append(head[7])
...
#                    print(head[0],head[2],head[3],head[4],head[5])
    ...
    (evt,evt_seismnum) = np.unique(evtcd,return_counts=True)
    evt = evt[evt_seismnum>minseismnum]
    ...
    for i in np.arange(number_grid):
        for j in np.arange(i,number_grid):
            ...
            if (deltaijkl < 10 and deltaijkl > 0.000001):
                row_reg.append(m)
                col_reg.append(i)
    ...
    col = []
    factor = []
    rhs0 = np.zeros(number_grid*number_event)
    count_event = np.zeros(number_grid*number_event)
    count = np.zeros(number_grid)
    row_line = 0
    for i in np.arange(len(grid_j)):
        if evtcd[i] in evt:
            idx = list(evt).index(evtcd[i])
            rhs0[grid_j[i]*number_event+idx] = rhs0[grid_j[i]*number_event+idx] + results[i]
            count_event[grid_j[i]*number_event+idx] = count_event[grid_j[i]*number_event+idx] + 1
            count[grid_j[i]] = count[grid_j[i]] + 1
    rhs0[count_event>0] = rhs0[count_event>0] / count_event[count_event>0]
    for i in np.arange(number_grid):
        n = np.sum(count_event[i*number_event:(i+1)*number_event]>0)
        for j in np.arange(number_event):
            if count_event[i*number_event+j]>0:
                row.append(row_line)
                col.append(i)
                factor.append(1.0/n)
                row.append(row_line)
                ...
                factor.append(1.0/n)
                row_line = row_line+1
    rhs0 = rhs0[count_event>0]
    A0 = coo_matrix((np.array(factor),(row,col)),shape=(len(rhs0),number_grid+number_event))
    ...
    for i in np.arange(number_event):
        row_evt.append(m)
        col_evt.append(i+number_grid)
        f=np.sum(factor_array[np.where(col_array==i+number_grid)[0]])
        value_evt.append(1-f)
        m = m + 1
    ...
    x[extract_index_0],istop,itn,r1norm,r2norm,anorm,acond,arnorm,xnorm,var = lsqr(A,rhs,atol=1e-12,btol=1e-12,damp=1)
...
def main():
    ...
    xhfilename = 'all_all.BHT.vel.snrS.xh'
    dirname = '/home/meichen/work1/SH_TOMO/Combined_xh'
    curpath = '/home/meichen/Research/SH_TOMO/USArray/temp2'
    minseismnum = 10
    average_or_root = 'average'

#    evt,seismnum = readxh(dirname='{}/finalxhfiles'.format(dirname),filename=xhfilename)

    for target_depth in np.hstack((np.arange(10,376,5),np.arange(455,1001,5))):
        filename = 'rp_all_deg3_win5.noprecursor.BHT.vel.snrS.xh.{:04d}'.format(int(target_depth))
        readbinary(filename=filename,dirname='{}/rp_all_files'.format(dirname),savepath=curpath,minseismnum=minseismnum,target_depth=target_depth,average_or_root=average_or_root)
