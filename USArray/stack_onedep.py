...
#                    print(head[0],head[2],head[3],head[4],head[5])
    ...
    event_term = event_term / num_event /2.0
    ...
    np.savetxt('{}/gridpointout_{:04d}_stack.txt'.format(savepath,int(target_depth)),gridpoint_terms,fmt="%8.3f %8.3f %8.5f %d")
...
def main():
    ...
    for target_depth in np.arange(10,1001,5):
        ...
        readbinary(filename=filename,dirname='{}/results_all_deg2_win5_dis_stack_ttcorr'.format(dirname),savepath=curpath,minseismnum=minseismnum,target_depth=target_depth,average_or_root=average_or_root,evt=evt,seismnum=seismnum)
