    ...
    with open("{}".format(filename),"rb") as fp:
        while True:
            ...
            if bihead:
                ...
                if ( np.abs(np.array(head[2])-target_depth) <0.1 and head[6]<2 and head[9]>5 and head[10]>5 and head[11]>5 and head[12]>2.5 and event_dep<35) and (a in evt):
                    if (max_or_root == 'average'):
                        results[head[0]] = results[head[0]] + head[5]
                    elif (max_or_root == 'max'):
                        results[head[0]] = results[head[0]] + head[13]
                    count[head[0]] = count[head[0]] + 1.0
            else:
                break
    results[count>0] = results[count>0] / count[count>0]
...
##
    ...
    for i in np.arange(number_grid):
        gridpoint_terms.append([gridpoints[i,0],gridpoints[i,1],results[i],count[i]])
...
def main():
    ...
    for target_depth in np.arange(450,451,5):
        filename = 'rp_all_deg2_win5.BHT.vel.snrS.xh.{:04d}'.format(int(target_depth))
        readbinary(filename=filename,dirname='{}/rp_all_files'.format(dirname),savepath=curpath,minseismnum=minseismnum,target_depth=target_depth,max_or_root=max_or_root,evt=evt,seismnum=seismnum)
